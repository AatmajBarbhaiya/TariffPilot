...
inventing a code.
  ...
  LLM path    LLM picks ONE of the nominated candidates (or abstains)    -> medium
  abstain     nothing qualifies / LLM abstains -> top-N "needs review"   -> low
...
def arbitrate(query, keyword_hits, vector_hits, toc):
    ...
    if not qualified:
        verdict = "out_of_scope" if (toc and not toc["in_scope"]) else "needs_review"
        return _result(verdict, None, "low", "abstain", [], cands[:config.TOP_N],
                       "No candidate cleared the evidence threshold.")
    ...
    # 3) Abstain — surface top-N for human review.
    return _result("needs_review", None, "low", "abstain", [], top_n,
                   "Signals did not converge; returning top candidates.")
...
def _result(decision, hs6, confidence, path, signals_agreed, candidates, reason):
    ...
    return {
        ...
        "path": path,                   # fast | llm | abstain | scope
